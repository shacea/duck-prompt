...
- **`config_settings.py`**: `ConfigSettings` 모델 정의. `config.yml` 파일의 내용을 구조화하고 유효성 검사 수행. API 키, 모델명 필드 추가됨. 필터링 관련 필드 유효성 검사 로직 포함.

### 2.2. 서비스 (`src/core/services/`)

- **`config_service.py`**: `ConfigService` 클래스. `config.yml` 파일을 로드, 저장, 업데이트하며 `ConfigSettings` 모델 사용. 기본 모델명/API 키 접근 메서드 제공. `utils.helpers.get_project_root` 사용.
...
- **`gemini_service.py`**: `GeminiService` 클래스. Gemini API 호출 및 응답 처리 로직 담당. `ConfigService`를 통해 API 키 및 모델명 사용. LangGraph 워크플로우 빌드 함수(`build_gemini_graph`) 포함.
- **`prompt_service.py`**: `PromptService` 클래스. 다양한 종류의 프롬프트(코드 강화, 메타) 생성 로직 담당. `os.path.relpath` 사용.
- **`state_service.py`**: `StateService` 클래스. `AppState` 모델을 사용하여 애플리케이션 상태를 JSON 파일로 저장, 로드, 가져오기, 내보내기, 백업, 복원하는 기능 제공. `utils.helpers.get_resource_path` 사용.
- **`template_service.py`**: `TemplateService` 클래스. 프롬프트 템플릿(`.md` 파일)을 로드, 저장, 삭제하는 기능 제공. `utils.helpers.get_resource_path` 사용.
- **`token_service.py`**: `TokenCalculationService` 클래스. GPT(`tiktoken`), Gemini(API), Claude(API) 모델별 토큰 계산 로직 담당. `ConfigService`를 통해 API 키 및 모델명 사용. API 호출 시 예외 처리 포함.
- **`xml_service.py`**: `XmlService` 클래스. XML 문자열을 파싱하여 `<code_changes>` 태그 내의 파일 생성/수정/삭제 작업을 수행. 경로 조작 방지 로직 포함.

### 2.3. LangGraph 상태 (`src/core/langgraph_state.py`)

- **`langgraph_state.py`**: `GeminiGraphState` TypedDict 정의. LangGraph 워크플로우에서 사용되는 상태 변수들을 타입 힌트로 명시.
...
- **`file_tree_controller.py`**: 파일 트리 뷰 관련 로직 담당 (폴더 선택, gitignore 로드/필터 업데이트, 트리 새로고침, 파일/폴더 이름 변경, 삭제, 체크 상태 변경 처리). `FilesystemService`, `ConfigService` 사용. SettingsDialog와 연동하여 gitignore 관리. 체크 상태 변경 시 문자 수 업데이트 트리거.
- **`main_controller.py`**: 애플리케이션 전반의 흐름 제어 (프로그램 리셋, 상태 표시줄 업데이트 - 문자/토큰 수). LLM 선택 및 모델명 변경 처리, `TokenCalculationService` 호출. 텍스트 변경 시 토큰 레이블 리셋.
- **`prompt_controller.py`**: 프롬프트 생성 및 클립보드 복사 관련 로직 담당. `PromptService` 사용. 코드 강화 모드 및 메타 프롬프트 모드 프롬프트 생성 로직 포함. 프롬프트 생성 후 토큰 계산 트리거.
...
- **역할**:
  ...
  - LangGraph Worker 스레드(`GeminiWorker`)를 사용하여 Gemini API 호출 및 응답 처리 (UI 비동기).
  - `summary_tab` 추가 및 관리.

### 3.5. `main_window_setup_ui.py`

- **기능**: `MainWindow`의 UI 위젯(메뉴, 버튼, 탭, 트리 뷰, 상태 표시줄 등) 생성 및 레이아웃 배치 담당. `CustomTextEdit`, `CustomTabBar`, `CheckBoxDelegate` 사용. 모드별 탭 구성 및 위젯 추가.

### 3.6. `main_window_setup_signals.py`

- **기능**: `MainWindow`의 UI 위젯 시그널과 해당 컨트롤러 슬롯(또는 `MainWindow` 메서드) 연결 담당. Gemini 전송, 메타 프롬프트 관련 버튼, 설정/상태 메뉴 액션, 텍스트 변경 시그널 연결 포함.

### 3.7. `settings_dialog.py`

- **기능**: 환경 설정(`config.yml`) 및 현재 프로젝트의 `.gitignore` 파일을 관리하는 다이얼로그 창 UI 및 로직 구현. `ConfigService`와 상호작용. UI에서 설정값을 읽어 `ConfigSettings` 모델로 변환 후 저장.
...
- **`app_*.spec`**: PyInstaller 빌드 설정 파일 (AMD64, ARM64). 의존성 및 포함 리소스 업데이트됨 (LangGraph, LangChain, 폰트 등).
- **`build.bat`**: Windows 환경 빌드 스크립트. `uv run` 사용하도록 업데이트됨.
- **`pyproject.toml`**: 프로젝트 메타데이터 및 빌드/도구 설정 (PEP 621). 의존성 업데이트됨 (`google-generativeai`, `anthropic`, `langgraph`, `langchain`, `langchain-google-genai` 추가). `ruff`, `black`, `mypy`, `pytest`, `coverage` 등 개발 도구 설정 포함.
