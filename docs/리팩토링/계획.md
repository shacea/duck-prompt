...
* **핵심 기능**: 코드 강화 빌더, 메타 프롬프트 빌더, PyQt5 GUI, 파일 탐색/선택(.gitignore 필터링), XML 파서, 템플릿/상태 관리, 설정 관리(.env, config.py), 유틸리티(토큰 계산 등).
...
2. **`src/core` (Core 계층)**: UI 독립적 핵심 비즈니스 로직. **Pydantic 모델** 적극 활용.
    * `models/`: **Pydantic 모델 정의** (`app_state.py`, `config_settings.py`, `xml_change.py` 등).
...
* **의존성 방향**: `UI Controllers` → `Core Services` → `Core Models / Utils / Config`. 역방향/건너뛰기 지양.
...
│       ├── __init__.py
│       └── helpers.py       # 범용 헬퍼 함수
├── unit_tests/              # 유닛 테스트 코드
│   ├── core/
│   │   ├── models/
│   │   │   └── test_app_state.py # Pydantic 모델 테스트
│   │   └── services/
...
graph TD
    ...
    App[src/app.py] -- Creates --> MainWindow
    App -- Configures --> Services

    Controllers -- Calls --> Services
    Services -- Uses --> CoreModels
...
sequenceDiagram
    ...
    StateService->>FileSystem: Write state_json to file
...
## 5.1. 1단계: 분석, 계획, 환경 설정 및 기본 구조 생성

* **목표**: 리팩토링 준비 완료, `src/` 구조 및 환경 설정, 초기 테스트 확보.
* **작업 목록**:
  * [x] 코드 베이스 분석, 문제점 식별, 리팩토링 범위 확정 (본 보고서).
  * [ ] Python 3.12 환경 구성 (`uv venv`).
  * [ ] `uv` 패키지 관리자 설정.
  * [ ] `requirements.txt` (`pydantic` 포함), `requirements-dev.txt` (`mypy` 포함) 작성 및 설치 (`uv pip install`).
  * [ ] `pyproject.toml` 생성 (uv, 빌드, ruff, black, **mypy** 설정 포함 - **pydantic 플러그인 활성화**).
  * [ ] Git 브랜치 생성 (`refactor/phase1-setup`).
  * [ ] `src/` 디렉토리 및 기본 하위 디렉토리(`core/models`, `core/services`, `ui`, `utils`) 생성, `__init__.py` 추가.
  * [ ] 모든 `.py` 파일을 `src/` 아래 적절한 위치로 이동 (`git mv`).
  * [ ] `app.py` 수정하여 `src/` 내부에서 실행되도록 진입점 변경.
  * [ ] 초기 `import` 오류 수정 (절대 경로 `from src...` 권장).
  * [ ] 코드 복잡도 측정 (`radon cc . -a`) 및 기록.
  * [ ] 기본 스모크 테스트 또는 수동 테스트 시나리오로 기능 동작 확인.
  * [ ] `tests/` 디렉토리 구성 및 `pytest` 기본 설정, 샘플 테스트 작성.
  * [ ] `README.md`에 개발 환경 설정 방법 추가.
* **완료 기준**: Python 3.12 환경에서 프로젝트 실행 가능, 모든 코드가 `src/` 내부에 위치, 기본적인 import 문제 해결, 애플리케이션 주요 기능 실행 가능, `ruff check .`, `black .` 실행 가능, `pytest` 초기 설정 완료.
* **검증 방법**: 수동 테스트, `uv pip list`, 애플리케이션 실행, `pytest tests/`.

## 5.2. 2단계: 모듈 분리 및 재구성 (Pydantic 도입)

* **목표**: UI, Core, Utils 계층 분리 완료. **Pydantic 모델 도입**. SRP 준수 향상 (**모듈별 토큰 수 관리 시작**).
* **작업 목록**:
  * [ ] Git 브랜치 생성 (`refactor/phase2-modularize`).
  * **Core 모듈 분리/리팩토링**:
    * [ ] `core/models/`에 `AppState`, `ConfigSettings` 등 **Pydantic 모델 정의**.
    * [ ] 각 `core/services/*_service.py` 파일로 기능 이동 및 UI 의존성 제거.
    * [ ] `state_service`, `config_service` 등에서 **Pydantic 모델 사용**하도록 로직 수정.
  * **Utils 모듈 분리/리팩토링**: `utils.py` -> `src/utils/helpers.py`.
  * **UI 모듈 분리/리팩토링**:
    * 커스텀 위젯 -> `src/ui/widgets/`.
    * 파일 탐색기 모델/로직 -> `src/ui/models/`, `src/ui/controllers/file_tree_controller.py`.
    * `main_window.py`: UI 정의만 남기고 이벤트 핸들러는 컨트롤러 호출로 변경.
    * `main_controller.py`: 책임을 분할하여 `src/ui/controllers/` 아래 여러 컨트롤러로 분산. Core 서비스 호출 로직 구현.
  * [ ] 절대 경로 import (`from src...`) 일관성 있게 적용.
  * [ ] 분리된 Core 모듈 (특히 Pydantic 모델 및 서비스)에 대한 유닛 테스트 작성 (`unit_tests/core/`). Mock 객체 활용.
  * [ ] 모듈 분리 시, 각 모듈의 토큰 수를 주기적으로 확인하여 **10000 토큰 가이드라인** 준수 노력. (필요시 추가 분리 고려)
  * [ ] CI 설정 (GitHub Actions 등) 추가: `pytest`, `ruff`, `black --check`, **`mypy src/`** 자동 실행.
* **완료 기준**: `src/` 하위 구조 계획대로 구현, Pydantic 모델 도입 완료, Core 모듈 UI 의존성 없음, Core 유닛 테스트 통과, CI 빌드 성공 (mypy 포함), 애플리케이션 기능 정상 동작, 분리된 모듈들이 토큰 수 가이드라인을 대체로 만족.
* **검증 방법**: 유닛 테스트 실행 (`pytest unit_tests/core`), CI 결과 확인, 수동 기능 테스트, 코드 리뷰 (SRP, 결합도, Pydantic 사용 적절성), `ruff check .`, `mypy src/`, 토큰 수 측정 도구(예: `tiktoken` 활용 스크립트) 사용 결과 확인.

## 5.3. 3단계: 최적화 및 개선 (타입 힌팅 완성)

* **목표**: 코드 품질 극대화, **타입 힌팅 완성 (`mypy` 통과)**, 문서화, 최종 테스트 및 검증.
* **작업 목록**:
  * [ ] Git 브랜치 생성 (`refactor/phase3-optimize`).
  * [ ] 전체 코드베이스 `black` 포매팅 적용.
  * [ ] 전체 코드베이스 `ruff` 린터 적용 및 모든 경고/오류 수정.
  * [ ] **모든 함수/메서드 타입 힌트 적용 완료 (`mypy src/` 통과 필수)**.
  * [ ] 복잡 로직 재검토 및 단순화 리팩토링.
  * [ ] 중복 코드 검토 및 제거.
  * [ ] 클래스/주요 함수 Docstring 추가 (Google 스타일 권장). 코드 내 주석 보강.
  * [ ] 유닛 테스트 커버리지 향상 (목표: 80% 이상). `ui/controllers`, `utils` 테스트 보강.
  * [ ] (선택) 성능 병목 구간 식별 및 최적화.
  * [ ] `README.md` 최종 업데이트 (구조, 사용법, 테스트 방법 등).
  * [ ] 최종 기능 테스트 및 QA.
  * [ ] 패키징 빌드 (`pyinstaller` 등) 재검증.
  * [ ] 리팩토링 브랜치들을 메인 브랜치로 병합.
* **완료 기준**: `black`, `ruff`, **`mypy`** 검사 통과, 타입 힌트 완료, Docstring/주석 충분, 테스트 커버리지 목표 달성, 모든 기능 안정적 동작, `README.md` 최신화, 패키징 성공.
* **검증 방법**: `black --check .`, `ruff check .`, `mypy src/`, `pytest --cov=src unit_tests/`, 코드 리뷰, 최종 수동 테스트, 빌드된 패키지 테스트.
...
* **테스트 구조**: `unit_tests/` 아래 `src/` 구조 미러링.
...
* **주요 테스트 케이스**:
  ...
  * `config_service`: 설정 로드 (Pydantic 모델 활용).
...
* **실행**: `uv run pytest unit_tests/`, 커버리지: `uv run pytest --cov=src unit_tests/`.
...
| 위험 요인                         | 대응 전략                                                                 |
| :-------------------------------- | :------------------------------------------------------------------------ |
| **Import 경로 충돌/오류**         | `src/` 루트 기반 절대 임포트(`from src...`) 표준화, `PYTHONPATH` 설정 확인. |
...
* **패키지 관리자**: **`uv`** 권장 및 기본 사용.
  ...
  * 실행: `uv run python src/app.py`, `uv run pytest ...`
* **의존성 파일**:
...
* **CI/CD**: GitHub Actions 활용. 워크플로우 정의 (`pytest`, `ruff`, `black --check`, `mypy src/` 실행).

---

## 9. README.md 업데이트 계획

* **프로젝트 구조**: 새로운 `src/` 기반 폴더 구조 설명 (core/models, core/services 포함).
* **설치 및 실행**: `uv` 기반 개발 환경 설정 및 실행 방법 명시.
* **기술 스택**: Python 3.12, PyQt5, Pydantic, mypy, uv, pytest, ruff, black 등 주요 기술 언급.
* **테스트**: `pytest` 실행 방법 및 `mypy` 타입 체크 방법 안내.
* **기여 방법 (선택)**: 코드 스타일 가이드, PR 템플릿 등.
