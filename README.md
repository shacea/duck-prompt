...
1.  **의존성 설치**
    ...
    - 기본 의존성 설치: `uv pip install -r requirements.txt`
      - **참고**: `requirements.txt`에는 Gemini/Claude 토큰 계산에 필요한 `google-generativeai`, `anthropic` 라이브러리가 포함되어 있습니다.
...
1. **메타 프롬프트 템플릿** 탭에 프롬프트를 감쌀 상위 템플릿을 작성합니다. (`[[user-prompt]]` 와 같이 변수 사용 가능)
2. **메타 사용자 입력** 탭에 실제로 사용자에게 요청할 내용을 적습니다.
3. **🚀 메타 프롬프트 생성** 버튼을 누르면 “메타 프롬프트 출력” 탭에서 결과를 볼 수 있습니다.
4. 필요 시, 추가 탭(`var-이름` 형태)을 생성하여 변수 내용을 입력하고, **🚀 최종 프롬프트 생성** 버튼으로 변수를 치환해 완성된 형태를 얻을 수도 있습니다.

### 디렉토리 트리 생성

- 코드 강화 빌더 모드에서 우측 상단의 **🌳 트리 생성** 버튼을 누르면, 체크된 파일/폴더를 기준으로 디렉토리 구조를 텍스트 형태로 생성합니다.
...
- **API 키**: Gemini, Anthropic API 키를 입력합니다. 토큰 계산에 사용됩니다. (Password 형태로 표시됨)
- **파일 필터링**:
