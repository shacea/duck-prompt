...
- **LLM별 토큰 계산**: GPT, Claude, Gemini 모델에 맞춰 토큰 수를 계산하여 프롬프트 길이를 가늠할 수 있도록 도와줍니다.
...
1.  **의존성 설치**
    ...
    - 기본 의존성 설치: `uv pip install -r requirements.txt`
      - Gemini/Claude 토큰 계산 API를 사용하려면 추가 설정이 필요합니다 (아래 [토큰 수 계산](#토큰-수-계산) 섹션 참고).
...
3. 체크박스를 통해 어떤 파일을 프롬프트에 포함할지 선택하세요.
   ...
   - 확실한 체크를 위해 폴더를 먼저 확장한 후 체크하시기 바랍니다. (클릭 시 체크/해제)
...
- **상태**로 전환하면, 현재 프로그램 상태(선택된 파일, 프롬프트 내용, 선택된 LLM 모델 등)를 파일로 저장하여 작업을 중단했다가 다시 이어서 할 수 있습니다.
  - 버튼을 통해 저장, 불러오기, 백업/복원 등의 동작을 할 수 있습니다.

### 토큰 수 계산

- 프로그램 하단 상태 표시줄에서 LLM 모델(Gemini, Claude, GPT)을 선택하고, 사용할 정확한 모델명을 입력할 수 있습니다.
- **토큰 자동 계산** 체크박스가 활성화되어 있으면, 텍스트 탭 내용이 변경될 때마다 선택된 모델 기준으로 토큰 수를 계산하여 보여줍니다.
- **GPT**: `tiktoken` 라이브러리를 사용하여 로컬에서 계산합니다. (o200k_base 인코딩 기준)
- **Gemini**: Google Generative AI API (`count_tokens`)를 호출하여 계산합니다.
  - **사전 설정**:
    - `google-generativeai` 라이브러리 설치 필요 (`uv pip install google-generativeai`).
    - Gemini API 키 필요. `src/config.yml` 파일 내 `gemini_api_key` 항목에 API 키를 입력합니다. (Google AI Studio 또는 Google Cloud Console에서 발급)
- **Claude**: Anthropic API (`messages.count_tokens`)를 호출하여 계산합니다.
  - **사전 설정**:
    - `anthropic` 라이브러리 설치 필요 (`uv pip install anthropic`).
    - Anthropic API 키 필요. `src/config.yml` 파일 내 `anthropic_api_key` 항목에 API 키를 입력합니다.
- **모델명 저장**: 입력된 모델명 옆의 **💾 모델 저장** 버튼을 누르면, 현재 선택된 LLM(Gemini 또는 Claude)의 기본 모델명으로 `src/config.yml` 파일에 저장됩니다. 다음에 프로그램을 시작할 때 이 모델명이 자동으로 로드됩니다.
- **오류 처리**: API 호출 실패 시 (인증 오류, 네트워크 문제, 설정 누락 등) 상태 표시줄에 "토큰 계산 오류" 메시지가 표시됩니다. 터미널/콘솔의 상세 오류 메시지를 확인하세요.
...
9.  **프로젝트 구조**:
    ...
    - 설정 파일은 `src/config.yml` 입니다. (기본 시스템 프롬프트 경로, LLM 모델명, API 키 등 설정)
...
2.  **토큰 계산이 잘못될 수도 있나요?**
    - **GPT**: `tiktoken` 라이브러리를 통해 추정 토큰 수를 계산하지만, 모델(또는 프롬프트) 종류에 따라 오차가 있을 수 있습니다.
    - **Claude/Gemini**: 각 서비스의 공식 API를 호출하므로 비교적 정확하지만, API 호출 방식이나 모델 업데이트에 따라 약간의 차이가 발생할 수 있습니다. API 호출 실패 시 계산이 불가능합니다.
...
5.  **Gemini/Claude 토큰 계산이 안 돼요.**
    - [토큰 수 계산](#토큰-수-계산) 섹션의 사전 설정 부분을 확인하세요. 필요한 라이브러리가 설치되어 있고, `src/config.yml` 파일에 해당 API 키(`gemini_api_key` 또는 `anthropic_api_key`)가 올바르게 입력되었는지 확인해야 합니다. 터미널/콘솔에 출력되는 상세 오류 메시지를 확인하여 문제를 진단하세요.
