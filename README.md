...
- **환경 설정**: **PostgreSQL 데이터베이스**에 저장된 설정을 GUI를 통해 확인하고, `.gitignore` 파일을 편집/저장할 수 있습니다. (DB 설정 직접 수정은 지원하지 않음)
...
10. **프로젝트 구조**:
    ...
    - **설정**: PostgreSQL 데이터베이스(`duck_agent`)의 `application_config`, `api_keys` 테이블에서 로드됩니다. (`src/config.yml` 파일은 더 이상 사용되지 않음)
